...
/// Crafting grid slots available at a crafting table (3x3). Without a table
/// only the first [`HAND_CRAFT_SIZE`] are usable.
pub const CRAFTING_SIZE: usize = 9;

/// Crafting grid slots available in the bare inventory screen (2x2, laid out
/// as the top-left corner of the 3x3 grid).
    ...
    pub fn insert(&mut self, stack: ItemStack, reg: &ItemRegistry) -> Option<ItemStack> {
        ...
        for slot in self.slots.iter_mut() {
            ...
            if let Some(existing) = slot {
                if existing.item == left.item && existing.count < max {
